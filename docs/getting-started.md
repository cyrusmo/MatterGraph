...
4. From the repository root, after `uv sync`, run: `uv run uvicorn mattergraph_api.main:app --reload --host 0.0.0.0 --port 8000` (the `mattergraph-api` package is installed in editable mode, so the `mattergraph_api` module resolves from `packages/mattergraph-api`.)
5. Open the web app under `apps/web` (see that folder’s [README](https://github.com/cyrusmo/MatterGraph/blob/main/apps/web/README.md)).
